...
void collectData(int samples, int delay_time){
   ...
   for(int i = 0; i < samples; i++){
      ...
      // Get temp, humidity & CO2 concentration from SCD-30
      if (scd30.dataReady()){
        //Serial.println("SCD-30 data available");
        if (!scd30.read()){ 
      ...
      if(writeSCD){
          Serial.print("write SCD - CO2: ");
          Serial.print(scd30.CO2);
          Serial.print(" temp: ");
          Serial.print(scd30.temperature);
          Serial.print(" humidity: ");
          Serial.println(scd30.relative_humidity);
          output_data.temp += scd30.temperature;
          output_data.humidity += scd30.relative_humidity;
          output_data.CO2 += scd30.CO2;  
      }
      if(writeAQI){
   ...
   if(SCDsamples != 0){
      output_data.temp /= SCDsamples;
      output_data.humidity /= SCDsamples;
      output_data.CO2 /= SCDsamples; 
   }
   if(AQIsamples != 0){
...
void setup() {
  ...
  // start SCD-30 CO2, temp & humdity sensor
  if (!scd30.begin()) {
...
void loop() {
  ...
  sensor.addField("CO2 Concentration (ppm)", output_data.CO2);
